...
def post_reports(): # функция отправляет запрос на создание нового отчета на сервере. Должна вызываться каждую минуту
    ...
    try:
        with requests.session() as session:
            response = session.post(url=api_url, headers=headers, json=id_to_post)  # отправляем запрос на создание отчета на сервер
        ...
        raise Exception("Ошибка при отправке запроса на получение отчета из сервера")  # Если не удалось установить соединение с сервером, выводим исключение
...
def get_reports():  # функция отправляет запрос на получение отчета из сервера. Должна вызываться каждую секунду
    ...
    for time_id in double_id_list.items():  # перебираем словарь по ключам id
        ...
        try:
            with requests.session() as session:
                response = session.get(api_url + "/" + id, headers=headers)  # отправляем запрос на получение отчета из сервера
